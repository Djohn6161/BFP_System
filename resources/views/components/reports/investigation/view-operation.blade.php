<!-- View Minimal Operation Modal -->
<div class="modal fade" id="viewMinimalOperationModal" tabindex="-1" aria-labelledby="viewMinimalOperationModalLabel"
    aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered modal-xl modal-dialog-scrollable">
        <div class="modal-content">
            <div class="modal-header pt-4 px-4 pb-1">
                <h3 class="modal-title fw-bolder text-primary">AFTER FIRE OPERATION REPORT</h3>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <hr>
            <div class="modal-body px-4 pb-4 pt-0">
                <table class="table table-striped">
                    <tr>
                        <th>Alarm received (Time):</th>
                        <td class="text-break">12:34 PM</td>
                    </tr>
                    <tr>
                        <th>Caller/Reported/Transmitted by:</th>
                        <td class="text-break">John Doe</td>
                    </tr>
                    <tr>
                        <th>Office/Address of the Caller:</th>
                        <td class="text-break">123 Main St, Springfield</td>
                    </tr>
                    <tr>
                        <th>Personnel on duty who receive the alarm:</th>
                        <td class="text-break">Lt. Jane Smith</td>
                    </tr>
                    <tr>
                        <th>Location:</th>
                        <td class="text-break">456 Elm St, Springfield</td>
                    </tr>
                </table>
                <br>
                <hr>
                <table class="table table-striped">
                    <thead>
                        <tr>
                            <th class="text-center align-middle">ENGINE DISPATCHED</th>
                            <th class="text-center align-middle">TIME DISPATCHED</th>
                            <th class="text-center align-middle">TIME ARRIVED AT FIRE SCENE</th>
                            <th class="text-center align-middle">RESPONSE TIME (TIME RECEIVED CALL - TIME ARRIVED AT FIRE SCENE) in minutes</th>
                            <th class="text-center align-middle">TIME RETURNED TO BASE</th>
                            <th class="text-center align-middle">WATER TANK REFILLED (GAL)</th>
                            <th class="text-center align-middle">GAS CONSUMED (L)</th>
                        </tr>
                    </thead>
                    <tbody>
                        <tr>
                            <td class="text-break">Engine 1</td>
                            <td class="text-break">12:35 PM</td>
                            <td class="text-break">12:45 PM</td>
                            <td class="text-break">10</td>
                            <td class="text-break">2:00 PM</td>
                            <td class="text-break">500</td>
                            <td class="text-break">50</td>
                        </tr>
                    </tbody>
                </table>
                <br>
                <hr>
                <table class="table table-striped">
                    <tr>
                        <th>Alarm Status:</th>
                        <td class="text-break">Active</td>
                    </tr>
                    <tr>
                        <th>First Responder:</th>
                        <td class="text-break">Firefighter A</td>
                    </tr>
                    <tr>
                        <th>Time/Date Under Control:</th>
                        <td class="text-break">May 1, 2024 | 1:30:00 PM</td>
                    </tr>
                    <tr>
                        <th>Time/Date Declared Fire Out:</th>
                        <td class="text-break">May 1, 2024 | 2:00:00 PM</td>
                    </tr>
                </table>
                <br>
                <hr>
                <h5 class="my-4 fw-bolder">Type of Occupancy (please specify):</h5>
                <div class="ps-5">
                    Residential - House / Single-family
                </div>
                <hr>
                <h5 class="my-4 fw-bolder">Approximate Distance of Fire Incident From Fire Station (Km):</h5>
                <div class="ps-5">
                    5
                </div>
                <hr>
                <h5 class="my-4 fw-bolder">General Description of the structure/s involved:</h5>
                <div class="ps-5">
                    Single-family home, 2 stories, wooden frame
                </div>
                <hr>
                <br>
                <h5 class="my-4 fw-bolder">Total Number of Casualty Reported:</h5>
                <table class="table table-striped ">
                    <tr>
                        <th> </th>
                        <th>Injured</th>
                        <th>Death</th>
                    </tr>
                    <tr>
                        <th>Civilian</th>
                        <td>2</td>
                        <td>0</td>
                    </tr>
                    <tr>
                        <th>FireFighter</th>
                        <td>0</td>
                        <td>0</td>
                    </tr>
                </table>
                <hr>
                <br>
                <h5 class="my-4 fw-bolder">Time Alarm Status Declared:</h5>
                <table class="table table-striped">
                    <tr>
                        <th>Alarm Status</th>
                        <th>Time</th>
                        <th>Fund Commander</th>
                    </tr>
                    <tr>
                        <td>First Alarm</td>
                        <td>12:35 PM</td>
                        <td>Captain John Doe</td>
                    </tr>
                </table>
                <hr>
                <br>
                <h5 class="my-4 fw-bolder">Breathing Apparatus Used:</h5>
                <table class="table table-striped ">
                    <tr>
                        <th>Qty.</th>
                        <th>Type/Kind</th>
                    </tr>
                    <tr>
                        <td>4</td>
                        <td>SCBA</td>
                    </tr>
                </table>
                <hr>
                <br>
                <h5 class="my-4 fw-bolder">Extinguishing Agent Used:</h5>
                <table class="table table-striped ">
                    <tr>
                        <th>Qty.</th>
                        <th>Type/Kind</th>
                    </tr>
                    <tr>
                        <td>2</td>
                        <td>Water</td>
                    </tr>
                </table>
                <hr>
                <br>
                <h5 class="my-4 fw-bolder">Rope and Ladder Used:</h5>
                <table class="table table-striped ">
                    <tr>
                        <th>Type.</th>
                        <th>Length</th>
                    </tr>
                    <tr>
                        <td>Rope</td>
                        <td>100 ft</td>
                    </tr>
                </table>
                <hr>
                <br>
                <h5 class="my-4 fw-bolder">Hose line Used:</h5>
                <table class="table table-striped ">
                    <tr>
                        <th>Nr.</th>
                        <th>TYPE/KIND</th>
                        <th>TOTAL ft</th>
                    </tr>
                    <tr>
                        <td>1</td>
                        <td>1.75-inch hose</td>
                        <td>200 ft</td>
                    </tr>
                </table>
                <hr>
                <br>
                <h5 class="my-4 fw-bolder">Duty Personnel at the Fire Scene:</h5>
                <table class="table table-striped">
                    <tr>
                        <th>Rank/Name</th>
                        <th>Designation</th>
                        <th>Remarks</th>
                    </tr>
                    <tr>
                        <td>Lt. Jane Smith</td>
                        <td>Commander</td>
                        <td>None</td>
                    </tr>
                </table>
                <h5 class="my-4 fw-bolder">Instruction/Sketch of the Fire Operation (Should Be Attached):
                    <span class="d-block" style="color: grey; font-style:italic; font-size:15px;">(Indicate the data frame, legend, location, north arrow and scale)</span>
                </h5>
                <div class="row">
                    <div class="col-lg-12">
                        <div class="">
                            <div class="card-body p-1">
                                <h3>No photos</h3>
                            </div>
                        </div>
                    </div>
                </div>
                <br>
                <hr>
                <h5 class="my-4 fw-bolder">Details(Narrative)</h5>
                <div class="ps-5">
                    The fire started in the kitchen and spread quickly to the rest of the house. The cause is suspected to be a grease fire.
                </div>
                <hr>
                <h5 class="my-4 fw-bolder">Problem/s Encountered during Operation:</h5>
                <div class="ps-5">
                    Limited water supply initially, resolved by connecting to a nearby hydrant.
                </div>
                <hr>
                <h5 class="my-4 fw-bolder">Observation/Recommendation</h5>
                <div class="ps-5">
                    Recommend additional training for handling kitchen fires and maintaining equipment readiness.
                </div>
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-secondary" data-bs-toggle="modal" data-bs-target="#viewMinimalModal{{ $investigation->id }}"><span><i class="ti ti-arrow-back"></i></span><span> Go Back</span></button>
                <a href="#" type="button" class="btn btn-warning"><i class="ti ti-printer"></i>Print</a>
            </div>
        </div>
        
    </div>
</div>

{{-- View Spot Operation Modal --}}
<div class="modal fade" id="viewSpotOperationModal" tabindex="-1" aria-labelledby="viewSpotOperationModalLabel"
    aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered modal-xl modal-dialog-scrollable">
        <div class="modal-content">
            <div class="modal-header pt-4 px-4 pb-1">
                <h3 class="modal-title fw-bolder text-primary">AFTER FIRE OPERATION REPORT</h3>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <hr>
            <div class="modal-body px-4 pb-4 pt-0">
                <table class="table table-striped">
                    <tr>
                        <th>Alarm received (Time):</th>
                        <td class="text-break">12:34 PM</td>
                    </tr>
                    <tr>
                        <th>Caller/Reported/Transmitted by:</th>
                        <td class="text-break">John Doe</td>
                    </tr>
                    <tr>
                        <th>Office/Address of the Caller:</th>
                        <td class="text-break">123 Main St, Springfield</td>
                    </tr>
                    <tr>
                        <th>Personnel on duty who receive the alarm:</th>
                        <td class="text-break">Lt. Jane Smith</td>
                    </tr>
                    <tr>
                        <th>Location:</th>
                        <td class="text-break">456 Elm St, Springfield</td>
                    </tr>
                </table>
                <br>
                <hr>
                <table class="table table-striped">
                    <thead>
                        <tr>
                            <th class="text-center align-middle">ENGINE DISPATCHED</th>
                            <th class="text-center align-middle">TIME DISPATCHED</th>
                            <th class="text-center align-middle">TIME ARRIVED AT FIRE SCENE</th>
                            <th class="text-center align-middle">RESPONSE TIME (TIME RECEIVED CALL - TIME ARRIVED AT FIRE SCENE) in minutes</th>
                            <th class="text-center align-middle">TIME RETURNED TO BASE</th>
                            <th class="text-center align-middle">WATER TANK REFILLED (GAL)</th>
                            <th class="text-center align-middle">GAS CONSUMED (L)</th>
                        </tr>
                    </thead>
                    <tbody>
                        <tr>
                            <td class="text-break">Engine 1</td>
                            <td class="text-break">12:35 PM</td>
                            <td class="text-break">12:45 PM</td>
                            <td class="text-break">10</td>
                            <td class="text-break">2:00 PM</td>
                            <td class="text-break">500</td>
                            <td class="text-break">50</td>
                        </tr>
                    </tbody>
                </table>
                <br>
                <hr>
                <table class="table table-striped">
                    <tr>
                        <th>Alarm Status:</th>
                        <td class="text-break">Active</td>
                    </tr>
                    <tr>
                        <th>First Responder:</th>
                        <td class="text-break">Firefighter A</td>
                    </tr>
                    <tr>
                        <th>Time/Date Under Control:</th>
                        <td class="text-break">May 1, 2024 | 1:30:00 PM</td>
                    </tr>
                    <tr>
                        <th>Time/Date Declared Fire Out:</th>
                        <td class="text-break">May 1, 2024 | 2:00:00 PM</td>
                    </tr>
                </table>
                <br>
                <hr>
                <h5 class="my-4 fw-bolder">Type of Occupancy (please specify):</h5>
                <div class="ps-5">
                    Residential - House / Single-family
                </div>
                <hr>
                <h5 class="my-4 fw-bolder">Approximate Distance of Fire Incident From Fire Station (Km):</h5>
                <div class="ps-5">
                    5
                </div>
                <hr>
                <h5 class="my-4 fw-bolder">General Description of the structure/s involved:</h5>
                <div class="ps-5">
                    Single-family home, 2 stories, wooden frame
                </div>
                <hr>
                <br>
                <h5 class="my-4 fw-bolder">Total Number of Casualty Reported:</h5>
                <table class="table table-striped ">
                    <tr>
                        <th> </th>
                        <th>Injured</th>
                        <th>Death</th>
                    </tr>
                    <tr>
                        <th>Civilian</th>
                        <td>2</td>
                        <td>0</td>
                    </tr>
                    <tr>
                        <th>FireFighter</th>
                        <td>0</td>
                        <td>0</td>
                    </tr>
                </table>
                <hr>
                <br>
                <h5 class="my-4 fw-bolder">Time Alarm Status Declared:</h5>
                <table class="table table-striped">
                    <tr>
                        <th>Alarm Status</th>
                        <th>Time</th>
                        <th>Fund Commander</th>
                    </tr>
                    <tr>
                        <td>First Alarm</td>
                        <td>12:35 PM</td>
                        <td>Captain John Doe</td>
                    </tr>
                </table>
                <hr>
                <br>
                <h5 class="my-4 fw-bolder">Breathing Apparatus Used:</h5>
                <table class="table table-striped ">
                    <tr>
                        <th>Qty.</th>
                        <th>Type/Kind</th>
                    </tr>
                    <tr>
                        <td>4</td>
                        <td>SCBA</td>
                    </tr>
                </table>
                <hr>
                <br>
                <h5 class="my-4 fw-bolder">Extinguishing Agent Used:</h5>
                <table class="table table-striped ">
                    <tr>
                        <th>Qty.</th>
                        <th>Type/Kind</th>
                    </tr>
                    <tr>
                        <td>2</td>
                        <td>Water</td>
                    </tr>
                </table>
                <hr>
                <br>
                <h5 class="my-4 fw-bolder">Rope and Ladder Used:</h5>
                <table class="table table-striped ">
                    <tr>
                        <th>Type.</th>
                        <th>Length</th>
                    </tr>
                    <tr>
                        <td>Rope</td>
                        <td>100 ft</td>
                    </tr>
                </table>
                <hr>
                <br>
                <h5 class="my-4 fw-bolder">Hose line Used:</h5>
                <table class="table table-striped ">
                    <tr>
                        <th>Nr.</th>
                        <th>TYPE/KIND</th>
                        <th>TOTAL ft</th>
                    </tr>
                    <tr>
                        <td>1</td>
                        <td>1.75-inch hose</td>
                        <td>200 ft</td>
                    </tr>
                </table>
                <hr>
                <br>
                <h5 class="my-4 fw-bolder">Duty Personnel at the Fire Scene:</h5>
                <table class="table table-striped">
                    <tr>
                        <th>Rank/Name</th>
                        <th>Designation</th>
                        <th>Remarks</th>
                    </tr>
                    <tr>
                        <td>Lt. Jane Smith</td>
                        <td>Commander</td>
                        <td>None</td>
                    </tr>
                </table>
                <h5 class="my-4 fw-bolder">Instruction/Sketch of the Fire Operation (Should Be Attached):
                    <span class="d-block" style="color: grey; font-style:italic; font-size:15px;">(Indicate the data frame, legend, location, north arrow and scale)</span>
                </h5>
                <div class="row">
                    <div class="col-lg-12">
                        <div class="">
                            <div class="card-body p-1">
                                <h3>No photos</h3>
                            </div>
                        </div>
                    </div>
                </div>
                <br>
                <hr>
                <h5 class="my-4 fw-bolder">Details(Narrative)</h5>
                <div class="ps-5">
                    The fire started in the kitchen and spread quickly to the rest of the house. The cause is suspected to be a grease fire.
                </div>
                <hr>
                <h5 class="my-4 fw-bolder">Problem/s Encountered during Operation:</h5>
                <div class="ps-5">
                    Limited water supply initially, resolved by connecting to a nearby hydrant.
                </div>
                <hr>
                <h5 class="my-4 fw-bolder">Observation/Recommendation</h5>
                <div class="ps-5">
                    Recommend additional training for handling kitchen fires and maintaining equipment readiness.
                </div>
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-secondary" data-bs-toggle="modal" data-bs-target="#viewSpotModal{{ $investigation->id }}"><span><i class="ti ti-arrow-back"></i></span><span> Go Back</span></button>
                <a href="#" type="button" class="btn btn-warning"><i class="ti ti-printer"></i>Print</a>
            </div>
        </div>
        
    </div>
</div>

{{-- View Progress Operation Modal --}}
<div class="modal fade" id="viewProgressOperationModal" tabindex="-1" aria-labelledby="viewProgressOperationModalLabel"
    aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered modal-xl modal-dialog-scrollable">
        <div class="modal-content">
            <div class="modal-header pt-4 px-4 pb-1">
                <h3 class="modal-title fw-bolder text-primary">AFTER FIRE OPERATION REPORT</h3>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <hr>
            <div class="modal-body px-4 pb-4 pt-0">
                <table class="table table-striped">
                    <tr>
                        <th>Alarm received (Time):</th>
                        <td class="text-break">12:34 PM</td>
                    </tr>
                    <tr>
                        <th>Caller/Reported/Transmitted by:</th>
                        <td class="text-break">John Doe</td>
                    </tr>
                    <tr>
                        <th>Office/Address of the Caller:</th>
                        <td class="text-break">123 Main St, Springfield</td>
                    </tr>
                    <tr>
                        <th>Personnel on duty who receive the alarm:</th>
                        <td class="text-break">Lt. Jane Smith</td>
                    </tr>
                    <tr>
                        <th>Location:</th>
                        <td class="text-break">456 Elm St, Springfield</td>
                    </tr>
                </table>
                <br>
                <hr>
                <table class="table table-striped">
                    <thead>
                        <tr>
                            <th class="text-center align-middle">ENGINE DISPATCHED</th>
                            <th class="text-center align-middle">TIME DISPATCHED</th>
                            <th class="text-center align-middle">TIME ARRIVED AT FIRE SCENE</th>
                            <th class="text-center align-middle">RESPONSE TIME (TIME RECEIVED CALL - TIME ARRIVED AT FIRE SCENE) in minutes</th>
                            <th class="text-center align-middle">TIME RETURNED TO BASE</th>
                            <th class="text-center align-middle">WATER TANK REFILLED (GAL)</th>
                            <th class="text-center align-middle">GAS CONSUMED (L)</th>
                        </tr>
                    </thead>
                    <tbody>
                        <tr>
                            <td class="text-break">Engine 1</td>
                            <td class="text-break">12:35 PM</td>
                            <td class="text-break">12:45 PM</td>
                            <td class="text-break">10</td>
                            <td class="text-break">2:00 PM</td>
                            <td class="text-break">500</td>
                            <td class="text-break">50</td>
                        </tr>
                    </tbody>
                </table>
                <br>
                <hr>
                <table class="table table-striped">
                    <tr>
                        <th>Alarm Status:</th>
                        <td class="text-break">Active</td>
                    </tr>
                    <tr>
                        <th>First Responder:</th>
                        <td class="text-break">Firefighter A</td>
                    </tr>
                    <tr>
                        <th>Time/Date Under Control:</th>
                        <td class="text-break">May 1, 2024 | 1:30:00 PM</td>
                    </tr>
                    <tr>
                        <th>Time/Date Declared Fire Out:</th>
                        <td class="text-break">May 1, 2024 | 2:00:00 PM</td>
                    </tr>
                </table>
                <br>
                <hr>
                <h5 class="my-4 fw-bolder">Type of Occupancy (please specify):</h5>
                <div class="ps-5">
                    Residential - House / Single-family
                </div>
                <hr>
                <h5 class="my-4 fw-bolder">Approximate Distance of Fire Incident From Fire Station (Km):</h5>
                <div class="ps-5">
                    5
                </div>
                <hr>
                <h5 class="my-4 fw-bolder">General Description of the structure/s involved:</h5>
                <div class="ps-5">
                    Single-family home, 2 stories, wooden frame
                </div>
                <hr>
                <br>
                <h5 class="my-4 fw-bolder">Total Number of Casualty Reported:</h5>
                <table class="table table-striped ">
                    <tr>
                        <th> </th>
                        <th>Injured</th>
                        <th>Death</th>
                    </tr>
                    <tr>
                        <th>Civilian</th>
                        <td>2</td>
                        <td>0</td>
                    </tr>
                    <tr>
                        <th>FireFighter</th>
                        <td>0</td>
                        <td>0</td>
                    </tr>
                </table>
                <hr>
                <br>
                <h5 class="my-4 fw-bolder">Time Alarm Status Declared:</h5>
                <table class="table table-striped">
                    <tr>
                        <th>Alarm Status</th>
                        <th>Time</th>
                        <th>Fund Commander</th>
                    </tr>
                    <tr>
                        <td>First Alarm</td>
                        <td>12:35 PM</td>
                        <td>Captain John Doe</td>
                    </tr>
                </table>
                <hr>
                <br>
                <h5 class="my-4 fw-bolder">Breathing Apparatus Used:</h5>
                <table class="table table-striped ">
                    <tr>
                        <th>Qty.</th>
                        <th>Type/Kind</th>
                    </tr>
                    <tr>
                        <td>4</td>
                        <td>SCBA</td>
                    </tr>
                </table>
                <hr>
                <br>
                <h5 class="my-4 fw-bolder">Extinguishing Agent Used:</h5>
                <table class="table table-striped ">
                    <tr>
                        <th>Qty.</th>
                        <th>Type/Kind</th>
                    </tr>
                    <tr>
                        <td>2</td>
                        <td>Water</td>
                    </tr>
                </table>
                <hr>
                <br>
                <h5 class="my-4 fw-bolder">Rope and Ladder Used:</h5>
                <table class="table table-striped ">
                    <tr>
                        <th>Type.</th>
                        <th>Length</th>
                    </tr>
                    <tr>
                        <td>Rope</td>
                        <td>100 ft</td>
                    </tr>
                </table>
                <hr>
                <br>
                <h5 class="my-4 fw-bolder">Hose line Used:</h5>
                <table class="table table-striped ">
                    <tr>
                        <th>Nr.</th>
                        <th>TYPE/KIND</th>
                        <th>TOTAL ft</th>
                    </tr>
                    <tr>
                        <td>1</td>
                        <td>1.75-inch hose</td>
                        <td>200 ft</td>
                    </tr>
                </table>
                <hr>
                <br>
                <h5 class="my-4 fw-bolder">Duty Personnel at the Fire Scene:</h5>
                <table class="table table-striped">
                    <tr>
                        <th>Rank/Name</th>
                        <th>Designation</th>
                        <th>Remarks</th>
                    </tr>
                    <tr>
                        <td>Lt. Jane Smith</td>
                        <td>Commander</td>
                        <td>None</td>
                    </tr>
                </table>
                <h5 class="my-4 fw-bolder">Instruction/Sketch of the Fire Operation (Should Be Attached):
                    <span class="d-block" style="color: grey; font-style:italic; font-size:15px;">(Indicate the data frame, legend, location, north arrow and scale)</span>
                </h5>
                <div class="row">
                    <div class="col-lg-12">
                        <div class="">
                            <div class="card-body p-1">
                                <h3>No photos</h3>
                            </div>
                        </div>
                    </div>
                </div>
                <br>
                <hr>
                <h5 class="my-4 fw-bolder">Details(Narrative)</h5>
                <div class="ps-5">
                    The fire started in the kitchen and spread quickly to the rest of the house. The cause is suspected to be a grease fire.
                </div>
                <hr>
                <h5 class="my-4 fw-bolder">Problem/s Encountered during Operation:</h5>
                <div class="ps-5">
                    Limited water supply initially, resolved by connecting to a nearby hydrant.
                </div>
                <hr>
                <h5 class="my-4 fw-bolder">Observation/Recommendation</h5>
                <div class="ps-5">
                    Recommend additional training for handling kitchen fires and maintaining equipment readiness.
                </div>
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-secondary" data-bs-toggle="modal" data-bs-target="#viewProgressModal{{ $investigation->id }}"><span><i class="ti ti-arrow-back"></i></span><span> Go Back</span></button>
                <a href="#" type="button" class="btn btn-warning"><i class="ti ti-printer"></i>Print</a>
            </div>
        </div>
        
    </div>
</div>

{{-- View Final Operation Modal --}}
<div class="modal fade" id="viewFinalOperationModal" tabindex="-1" aria-labelledby="viewFinalOperationModalLabel"
    aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered modal-xl modal-dialog-scrollable">
        <div class="modal-content">
            <div class="modal-header pt-4 px-4 pb-1">
                <h3 class="modal-title fw-bolder text-primary">AFTER FIRE OPERATION REPORT</h3>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <hr>
            <div class="modal-body px-4 pb-4 pt-0">
                <table class="table table-striped">
                    <tr>
                        <th>Alarm received (Time):</th>
                        <td class="text-break">12:34 PM</td>
                    </tr>
                    <tr>
                        <th>Caller/Reported/Transmitted by:</th>
                        <td class="text-break">John Doe</td>
                    </tr>
                    <tr>
                        <th>Office/Address of the Caller:</th>
                        <td class="text-break">123 Main St, Springfield</td>
                    </tr>
                    <tr>
                        <th>Personnel on duty who receive the alarm:</th>
                        <td class="text-break">Lt. Jane Smith</td>
                    </tr>
                    <tr>
                        <th>Location:</th>
                        <td class="text-break">456 Elm St, Springfield</td>
                    </tr>
                </table>
                <br>
                <hr>
                <table class="table table-striped">
                    <thead>
                        <tr>
                            <th class="text-center align-middle">ENGINE DISPATCHED</th>
                            <th class="text-center align-middle">TIME DISPATCHED</th>
                            <th class="text-center align-middle">TIME ARRIVED AT FIRE SCENE</th>
                            <th class="text-center align-middle">RESPONSE TIME (TIME RECEIVED CALL - TIME ARRIVED AT FIRE SCENE) in minutes</th>
                            <th class="text-center align-middle">TIME RETURNED TO BASE</th>
                            <th class="text-center align-middle">WATER TANK REFILLED (GAL)</th>
                            <th class="text-center align-middle">GAS CONSUMED (L)</th>
                        </tr>
                    </thead>
                    <tbody>
                        <tr>
                            <td class="text-break">Engine 1</td>
                            <td class="text-break">12:35 PM</td>
                            <td class="text-break">12:45 PM</td>
                            <td class="text-break">10</td>
                            <td class="text-break">2:00 PM</td>
                            <td class="text-break">500</td>
                            <td class="text-break">50</td>
                        </tr>
                    </tbody>
                </table>
                <br>
                <hr>
                <table class="table table-striped">
                    <tr>
                        <th>Alarm Status:</th>
                        <td class="text-break">Active</td>
                    </tr>
                    <tr>
                        <th>First Responder:</th>
                        <td class="text-break">Firefighter A</td>
                    </tr>
                    <tr>
                        <th>Time/Date Under Control:</th>
                        <td class="text-break">May 1, 2024 | 1:30:00 PM</td>
                    </tr>
                    <tr>
                        <th>Time/Date Declared Fire Out:</th>
                        <td class="text-break">May 1, 2024 | 2:00:00 PM</td>
                    </tr>
                </table>
                <br>
                <hr>
                <h5 class="my-4 fw-bolder">Type of Occupancy (please specify):</h5>
                <div class="ps-5">
                    Residential - House / Single-family
                </div>
                <hr>
                <h5 class="my-4 fw-bolder">Approximate Distance of Fire Incident From Fire Station (Km):</h5>
                <div class="ps-5">
                    5
                </div>
                <hr>
                <h5 class="my-4 fw-bolder">General Description of the structure/s involved:</h5>
                <div class="ps-5">
                    Single-family home, 2 stories, wooden frame
                </div>
                <hr>
                <br>
                <h5 class="my-4 fw-bolder">Total Number of Casualty Reported:</h5>
                <table class="table table-striped ">
                    <tr>
                        <th> </th>
                        <th>Injured</th>
                        <th>Death</th>
                    </tr>
                    <tr>
                        <th>Civilian</th>
                        <td>2</td>
                        <td>0</td>
                    </tr>
                    <tr>
                        <th>FireFighter</th>
                        <td>0</td>
                        <td>0</td>
                    </tr>
                </table>
                <hr>
                <br>
                <h5 class="my-4 fw-bolder">Time Alarm Status Declared:</h5>
                <table class="table table-striped">
                    <tr>
                        <th>Alarm Status</th>
                        <th>Time</th>
                        <th>Fund Commander</th>
                    </tr>
                    <tr>
                        <td>First Alarm</td>
                        <td>12:35 PM</td>
                        <td>Captain John Doe</td>
                    </tr>
                </table>
                <hr>
                <br>
                <h5 class="my-4 fw-bolder">Breathing Apparatus Used:</h5>
                <table class="table table-striped ">
                    <tr>
                        <th>Qty.</th>
                        <th>Type/Kind</th>
                    </tr>
                    <tr>
                        <td>4</td>
                        <td>SCBA</td>
                    </tr>
                </table>
                <hr>
                <br>
                <h5 class="my-4 fw-bolder">Extinguishing Agent Used:</h5>
                <table class="table table-striped ">
                    <tr>
                        <th>Qty.</th>
                        <th>Type/Kind</th>
                    </tr>
                    <tr>
                        <td>2</td>
                        <td>Water</td>
                    </tr>
                </table>
                <hr>
                <br>
                <h5 class="my-4 fw-bolder">Rope and Ladder Used:</h5>
                <table class="table table-striped ">
                    <tr>
                        <th>Type.</th>
                        <th>Length</th>
                    </tr>
                    <tr>
                        <td>Rope</td>
                        <td>100 ft</td>
                    </tr>
                </table>
                <hr>
                <br>
                <h5 class="my-4 fw-bolder">Hose line Used:</h5>
                <table class="table table-striped ">
                    <tr>
                        <th>Nr.</th>
                        <th>TYPE/KIND</th>
                        <th>TOTAL ft</th>
                    </tr>
                    <tr>
                        <td>1</td>
                        <td>1.75-inch hose</td>
                        <td>200 ft</td>
                    </tr>
                </table>
                <hr>
                <br>
                <h5 class="my-4 fw-bolder">Duty Personnel at the Fire Scene:</h5>
                <table class="table table-striped">
                    <tr>
                        <th>Rank/Name</th>
                        <th>Designation</th>
                        <th>Remarks</th>
                    </tr>
                    <tr>
                        <td>Lt. Jane Smith</td>
                        <td>Commander</td>
                        <td>None</td>
                    </tr>
                </table>
                <h5 class="my-4 fw-bolder">Instruction/Sketch of the Fire Operation (Should Be Attached):
                    <span class="d-block" style="color: grey; font-style:italic; font-size:15px;">(Indicate the data frame, legend, location, north arrow and scale)</span>
                </h5>
                <div class="row">
                    <div class="col-lg-12">
                        <div class="">
                            <div class="card-body p-1">
                                <h3>No photos</h3>
                            </div>
                        </div>
                    </div>
                </div>
                <br>
                <hr>
                <h5 class="my-4 fw-bolder">Details(Narrative)</h5>
                <div class="ps-5">
                    The fire started in the kitchen and spread quickly to the rest of the house. The cause is suspected to be a grease fire.
                </div>
                <hr>
                <h5 class="my-4 fw-bolder">Problem/s Encountered during Operation:</h5>
                <div class="ps-5">
                    Limited water supply initially, resolved by connecting to a nearby hydrant.
                </div>
                <hr>
                <h5 class="my-4 fw-bolder">Observation/Recommendation</h5>
                <div class="ps-5">
                    Recommend additional training for handling kitchen fires and maintaining equipment readiness.
                </div>
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-secondary" data-bs-toggle="modal" data-bs-target="#viewFinalModal{{ $investigation->id }}"><span><i class="ti ti-arrow-back"></i></span><span> Go Back</span></button>
                <a href="#" type="button" class="btn btn-warning"><i class="ti ti-printer"></i>Print</a>
            </div>
        </div>
        
    </div>
</div>
