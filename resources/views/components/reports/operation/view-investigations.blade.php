<div class="modal fade" tabindex="-1" id="viewMinimalOperationModal{{ $operation->id }}">
    <div class="modal-dialog modal-xl modal-dialog-scrollable">
        <div class="modal-content">
            <div class="modal-header pt-4 px-4 pb-1">
                <h3>klnafkl</h3>
            </div>
            <hr>
            <div class="modal-body px-4 pb-4 pt-0">
                <p>lmokef</p>
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-secondary" data-bs-toggle="modal" data-bs-target="#viewOperationModal"><span><i class="ti ti-arrow-back"></i></span><span> Go Back</span></button>
            </div>
        </div>
    </div>
</div>
{{-- <x-reports.investigation.view-operation :act="'minimal'" :operation="$investigation->afor" :investigation="$investigation" :responses="$responses" :personnels="$personnels"></x-reports.investigation.view-operation> --}}

<div class="modal fade" tabindex="-1" id="viewSpotOperationModal{{ $operation->id }}">
    <div class="modal-dialog modal-xl modal-dialog-scrollable">
        <div class="modal-content">
            <div class="modal-header pt-4 px-4 pb-1">
                <h3>meklkfis</h3>
            </div>
            <hr>
            <div class="modal-body pt-4 px-4 pt-0">
                <p>kfkosefvoe</p>
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-secondary" data-bs-toggle="modal" data-bs-target="#viewOperationModal"><span><i class="ti ti-arrow-back"></i></span><span> Go Back</span></button>
            </div>
        </div>
    </div>
</div>
<div class="modal fade" tabindex="-1" id="viewProgressOperationModal{{ $operation->id }}">
    <div class="modal-dialog modal-xl modal-dialog-scrollable">
        <div class="modal-content">
            <div class="modal-header pt-4 px-4 pb-1">
                <h3>meklkfis</h3>
            </div>
            <hr>
            <div class="modal-body pt-4 px-4 pt-0">
                <p>kfkosefvoe</p>
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-secondary" data-bs-toggle="modal" data-bs-target="#viewOperationModal"><span><i class="ti ti-arrow-back"></i></span><span> Go Back</span></button>
            </div>
        </div>
    </div>
</div>
<div class="modal fade" tabindex="-1" id="viewFinalOperationModal{{ $operation->id }}">
    <div class="modal-dialog modal-xl modal-dialog-scrollable">
        <div class="modal-content">
            <div class="modal-header pt-4 px-4 pb-1">
                <h3>meklkfis</h3>
            </div>
            <hr>
            <div class="modal-body pt-4 px-4 pt-0">
                <p>kfkosefvoe</p>
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-secondary" data-bs-toggle="modal" data-bs-target="#viewOperationModal"><span><i class="ti ti-arrow-back"></i></span><span> Go Back</span></button>
            </div>
        </div>
    </div>
</div>
